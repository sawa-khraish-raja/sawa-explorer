...
export default function AdminBookings() {
  ...
  const filteredAdventureBookings = adventureBookings.filter(booking => 
    booking.city?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    booking.traveler_email?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const totalRevenueServices = serviceBookings.filter(b => b.status === 'confirmed').reduce((sum, b) => sum + (b.total_price || 0), 0);

  const confirmedAdventures = adventureBookings.filter(b => b.status === 'confirmed').length;
  const totalRevenueAdventures = adventureBookings.filter(b => b.status === 'confirmed').reduce((sum, b) => sum + (b.total_price || 0), 0);
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className="text-2xl font-bold text-purple-600">${totalRevenueServices.toFixed(0)}</p>
                </CardContent>
              </Card>
            </div>
                    ...
                    <div key={booking.id} className="p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500 mt-1 truncate">{booking.traveler_email}</p>
                        </div>
                        <Button
                          size="sm"
                          variant="outline"
          ...
          <TabsContent value="adventures" className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
              ...
              <Card>
                <CardContent className="p-4">
                  <p className="text-sm text-gray-600">Revenue</p>
                  <p className="text-2xl font-bold text-purple-600">${totalRevenueAdventures.toFixed(0)}</p>
                </CardContent>
              </Card>
            </div>
                ...
                <div className="hidden md:block overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-purple-50 border-b border-purple-200">
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Booking ID</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Adventure</th>
                ...
                <div className="md:hidden divide-y">
                  {filteredAdventureBookings.map(booking => (
                    <div key={booking.id} className="p-4 space-y-3 bg-purple-50/30">
                      ...
                      <div className="text-xs text-gray-500">
                        {format(new Date(booking.start_date), 'MMM d, yyyy')} • {booking.number_of_adults || 1} guest(s)
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">{format(new Date(selectedBooking.start_date), 'PPP')}</p>
                  </div>
                  {!selectedBooking.adventure_id && (
                    <div>
                      <p className="text-sm text-gray-600">Check-out</p>
                      <p className="font-semibold">{format(new Date(selectedBooking.end_date), 'PPP')}</p>
                    </div>
                  )}
                  <div>
                    <p className="text-sm text-gray-600">Guests</p>
                    <p className="font-semibold">{selectedBooking.number_of_adults} adults{selectedBooking.number_of_children ? `, ${selectedBooking.number_of_children} children` : ''}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Total Price</p>
                    <p className="font-semibold text-lg text-green-600">${selectedBooking.total_price || 0}</p>
                  </div>
                </div>
