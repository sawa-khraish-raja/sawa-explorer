...
export default function HostDashboard() {
  ...
  const { data: myBookings = [], isLoading: isLoadingMy } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const relevantConvos = myConversations.filter(conv => 
          conv.host_emails && 
          Array.isArray(conv.host_emails) && 
          conv.host_emails.includes(user.email) &&
  ...
  const unreadCount = conversations.filter(c => 
    Array.isArray(c.unread_by_hosts) && c.unread_by_hosts.includes(user.email)
  ).length;
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-purple-600">${stats.earnings.toFixed(0)}</p>
                    </div>
                      ...
                      <CardHeader className="pb-3">
                        <div className="flex items-start justify-between">
                          ...
                          <Badge className={cn(
                            ...
                            {booking.status === 'confirmed' || booking.state === 'confirmed' ? 'Confirmed' : 'Pending'}
                          </Badge>
                        </div>
                      </CardHeader>
                      <CardContent className="space-y-3">
                        ...
                        <Button
                          onClick={() => {
                            if (booking.conversation_id) {
                              navigate(createPageUrl(`Messages?conversation_id=${booking.conversation_id}`));
                            } else {
