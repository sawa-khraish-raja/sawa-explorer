...
import {
  TrendingUp, Users, DollarSign, Target, Activity, Globe,
  BarChart3, Zap, RefreshCw, Eye, Clock, ArrowUpRight,
  ArrowDownRight, Loader2, CheckCircle2, AlertCircle,
  MapPin, Smartphone, Monitor, Tablet, Calendar, TrendingDown
} from 'lucide-react';
...
export default function MarketingDashboard() {
  ...
  const campaignMetrics = useMemo(() => {
    ...
    const avgPerformance = campaigns.length > 0
  ...
  const conversionFunnel = useMemo(() => {
    ...
    return [
      ...
      { stage: 'Booking Started', count: bookingStarted, percentage: pageViews > 0 ? (bookingStarted / pageViews) * 100 : 0 },
      { stage: 'Completed', count: bookingCompleted, percentage: pageViews > 0 ? (bookingCompleted / pageViews) * 100 : 0 }
    ];
  }, [conversions]);
              ...
              <div className="space-y-3">
                {cityData.slice(0, 5).map((city) => (
                  <div key={city.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <Badge className={
                      city.market_potential === 'high' ? 'bg-green-100 text-green-800' :
                      city.market_potential === 'medium' ? 'bg-yellow-100 text-yellow-800' :
  ...
  return (
    <Card className="hover:shadow-lg transition-shadow">
      <CardContent className={small ? 'p-4' : 'p-4 sm:p-6'}>
        ...
        <div className={small ? 'text-xl sm:text-2xl font-bold' : 'text-2xl sm:text-3xl font-bold mb-1'}>
          {value}
        </div>
        {change && TrendIcon && (
