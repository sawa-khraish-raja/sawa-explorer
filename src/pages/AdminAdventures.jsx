...
export default function AdminAdventures() {
  ...
  const { data: adventures = [], isLoading: adventuresLoading } = useQuery({
    ...
    queryFn: async () => {
      return await base44.entities.Adventure.list('-created_date');
    },
  ...
  const approveMutation = useMutation({
    mutationFn: async (adventureId) => {
      return await base44.entities.Adventure.update(adventureId, {
  ...
  const rejectMutation = useMutation({
    mutationFn: async ({ adventureId, reason }) => {
      return await base44.entities.Adventure.update(adventureId, {
  ...
  // Create admin adventure mutation
  const createAdminAdventureMutation = useMutation({
    mutationFn: async (adventureData) => {
      return await base44.entities.Adventure.create({
        ...adventureData,
  ...
  const filteredAdventures = (() => {
    let filtered = activeTab === 'all' ? adventures :
      ...
      <div className="space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-white shadow-md p-1">
            <TabsTrigger value="pending" className={cn("data-[state=active]:bg-[#9933CC] data-[state=active]:text-white")}>
              Pending ({pending.length})
            </TabsTrigger>
            <TabsTrigger value="approved" className={cn("data-[state=active]:bg-[#9933CC] data-[state=active]:text-white")}>
              Approved ({approved.length})
            </TabsTrigger>
            <TabsTrigger value="rejected" className={cn("data-[state=active]:bg-[#9933CC] data-[state=active]:text-white")}>
              Rejected ({rejected.length})
            </TabsTrigger>
            <TabsTrigger value="all" className={cn("data-[state=active]:bg-[#9933CC] data-[state=active]:text-white")}>
            ...
            {filteredAdventures.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredAdventures.map((adventure) => (
                  <Card key={adventure.id} className="hover:shadow-xl transition-shadow border-2 border-[#E6E6FF]">
                    <div className="relative aspect-[4/3] overflow-hidden">
                      ...
                      <div className="absolute top-3 right-3 flex gap-2">
                        <Badge className={
                          adventure.approval_status === 'approved' ? 'bg-green-500' :
                          adventure.approval_status === 'pending' ? 'bg-yellow-500' :
                          'bg-red-500'
                        }>
                      ...
                      <div className="absolute top-3 left-3">
                        <Badge className={
                          adventure.added_by_type === 'admin' ? 'bg-purple-600' :
                          adventure.added_by_type === 'office' ? 'bg-blue-600' :
                          'bg-green-600'
                        }>
                          {adventure.added_by_type === 'admin' ? 'Admin' :
                           adventure.added_by_type === 'office' ? 'Office' :
                           'Host'}
                        </Badge>
                      </div>
                    </div>
                    ...
                    <CardContent className="p-5">
                      ...
                      <p className="text-sm text-gray-600 line-clamp-2 mb-4">{adventure.description}</p>
                      ...
                      <div className="space-y-2 mb-4">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-600">
                          <DollarSign className="w-4 h-4 text-[#9933CC]" />
                          ${adventure.traveler_total_price} (Traveler pays)
                        </div>
                      </div>
        ...
        {/* Rejection Dialog */}
        <Dialog open={!!reviewingAdventure} onOpenChange={() => {
          setReviewingAdventure(null);
          setRejectionReason('');
        }}>
          <DialogContent>
