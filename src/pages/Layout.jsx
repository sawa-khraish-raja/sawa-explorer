...
import React, { createContext, useContext, useEffect, useCallback, useRef, useState, useMemo, memo } from "react";
import { Link, useLocation, useNavigate } from "react-router-dom";
import { createPageUrl } from "@/utils";
import { Menu, User, LogOut, LayoutDashboard, MessageSquare, Youtube, Instagram, Briefcase, Home, Info, Compass, Bell, Globe, ChevronLeft, MapPin, Users, BarChart3 } from "lucide-react";
...
const DesktopNav = memo(({ isHost, isOfficeOnly, hasAdminRole, isActivePage, createPageUrl, t, location, officeDashboardBaseUrl, adminDashboardBaseUrl }) => {
  if (isOfficeOnly) {
    return (
      <Link to={createPageUrl("OfficeDashboard")} className={cn("px-4 py-2.5 text-[15px] font-medium rounded-full transition-all duration-150", location.pathname.startsWith(officeDashboardBaseUrl) ? "text-gray-900" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900")}>
        {t('nav.office')}
      </Link>
    );
  }
  ...
  if (isHost && !hasAdminRole) {
    return (
      <>
        <Link to={createPageUrl("HostDashboard")} className={cn("px-4 py-2.5 text-[15px] font-medium rounded-full transition-all duration-150", isActivePage("HostDashboard") ? "text-gray-900" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900")}>
          {t('nav.my_dashboard')}
        </Link>
        <Link to={createPageUrl("Adventures")} className={cn("px-4 py-2.5 text-[15px] font-medium rounded-full transition-all duration-150", isActivePage("Adventures") ? "text-gray-900" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900")}>
          {t('nav.adventures')}
        </Link>
        <Link to={createPageUrl("ForumHome")} className={cn("px-4 py-2.5 text-[15px] font-medium rounded-full transition-all duration-150", isActivePage("ForumHome") ? "text-gray-900" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900")}>
          Community
        </Link>
      </>
    );
  ...
  if (hasAdminRole) {
    return (
      <>
        <Link to={createPageUrl("Home")} className={cn("px-4 py-2.5 text-[15px] font-medium rounded-full transition-all duration-150", isActivePage("Home") ? "text-gray-900" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900")}>
          {t('nav.home')}
        </Link>
        <Link to={createPageUrl("Destinations")} className={cn("px-4 py-2.5 text-[15px] font-medium rounded-full transition-all duration-150", isActivePage("Destinations") ? "text-gray-900" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900")}>
          Destinations
        </Link>
        <Link to={createPageUrl("ForumHome")} className={cn("px-4 py-2.5 text-[15px] font-medium rounded-full transition-all duration-150", isActivePage("ForumHome") ? "text-gray-900" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900")}>
          Community
        </Link>
        <Link to={createPageUrl("About")} className={cn("px-4 py-2.5 text-[15px] font-medium rounded-full transition-all duration-150", isActivePage("About") ? "text-gray-900" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900")}>
          {t('nav.about')}
        </Link>
      </>
    );
...
const AppContent = memo(({ children, currentPageName }) => {
  ...
  const userRoles = useMemo(() => {
    if (!user) return { hasAdminRole: false, hasOfficeRole: false, isHost: false, isMarketing: false };
  ...
  const isHostDashboardWithConvo = location.pathname.startsWith(hostDashboardBaseUrl) && location.search?.includes('conversation_id');
  ...
  const isInConversation = useMemo(() =>
    (isMessagesPage || isHostDashboardWithConvo || isOfficeDashboardPage) && location.search?.includes('conversation_id'),
    [isMessagesPage, isHostDashboardWithConvo, isOfficeDashboardPage, location.search]
  );

  const hideFooter = isMessagesPage || isHostDashboardWithConvo || isOfficeDashboardPage || isAdminDashboardPage;

  const isActivePage = useCallback((pageName) => location.pathname === createPageUrl(pageName), [location.pathname]);
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col bg-white pb-[env(safe-area-inset-bottom)]" dir={language === 'ar' || language === 'he' ? 'rtl' : 'ltr'}>
        <style>{`
            :root {
                ...
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  {shouldShowBackButton() && (
                    <button
                      ...
                      type="button"
                    >
                      <ChevronLeft className={cn("w-5 h-5 text-gray-700 group-hover:text-gray-900", language === 'ar' && 'rotate-180')} />
                    </button>
                  )}
                  <Link
                    to={
                      isOfficeOnly ? createPageUrl("OfficeDashboard") :
                      isHost ? createPageUrl("HostDashboard") :
                      createPageUrl("Home")
                    }
                  ...
                  {showLoadingSkeleton ? (
                    ...
                    <>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-64 bg-white shadow-xl border border-gray-200 rounded-2xl p-2 mt-2">
                          ...
                          <DropdownMenuItem asChild>
                            <Link to={createPageUrl("UserProfile")} className="cursor-pointer flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 rounded-lg">
                              <User className="w-4 h-4" />
                              <span className="text-sm font-medium">{t('userMenu.my_profile')}</span>
                            </Link>
                          </DropdownMenuItem>
                          
                          {hasAdminRole && (
                            <DropdownMenuItem asChild>
                              <Link to={createPageUrl("AdminDashboard")} className="cursor-pointer flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 rounded-lg">
                                <LayoutDashboard className="w-4 h-4" />
                                <span className="text-sm font-medium">{t('userMenu.admin_panel')}</span>
                              </Link>
                            </DropdownMenuItem>
                          )}
                          ...
                          {isMarketing && (
                            <DropdownMenuItem asChild>
                              <Link to={createPageUrl("MarketingDashboard")} className="cursor-pointer flex items-center gap-3 px-4 py-2.5 hover:bg-purple-50 rounded-lg">
                                <BarChart3 className="w-4 h-4 text-purple-600" />
                                <span className="text-sm font-medium text-purple-600">Marketing Dashboard</span>
                              </Link>
                            </DropdownMenuItem>
                          )}
                          ...
                          {isOfficeOnly && (
                            <DropdownMenuItem asChild>
                              <Link to={createPageUrl("OfficeDashboard")} className="cursor-pointer flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 rounded-lg">
                                <LayoutDashboard className="w-4 h-4" />
                                <span className="text-sm font-medium">{t('userMenu.office_dashboard')}</span>
                              </Link>
                            </DropdownMenuItem>
                          )}
              ...
              <div className="flex items-center gap-1">
                {shouldShowBackButton() && (
                  <button
                    ...
                    type="button"
                  >
                    <ChevronLeft className={cn("w-8 h-8 text-gray-900", language === 'ar' && 'rotate-180')} />
                  </button>
                )}
                ...
                {showLoadingSkeleton ? (
                  ...
                  <>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-56 bg-white shadow-xl border border-gray-200 rounded-xl p-2 mt-2">
                        ...
                        <DropdownMenuItem asChild>
                          <Link to={createPageUrl("UserProfile")} className="cursor-pointer flex items-center gap-3 px-3 py-2 hover:bg-gray-50 rounded-lg">
                            <User className="w-4 h-4" />
                        ...
                        <DropdownMenuItem asChild>
                          <Link to={createPageUrl("ForumHome")} className="cursor-pointer flex items-center gap-3 px-3 py-2 hover:bg-gray-50 rounded-lg">
                        ...
                        <DropdownMenuItem asChild>
                          <Link to={createPageUrl("About")} className="cursor-pointer flex items-center gap-3 px-3 py-2 hover:bg-gray-50 rounded-lg">
                        ...
                        {hasAdminRole && (
                          <DropdownMenuItem asChild>
                            <Link to={createPageUrl("AdminDashboard")} className="cursor-pointer flex items-center gap-3 px-3 py-2 hover:bg-purple-50 rounded-lg">
                  ...
                  <>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-56 bg-white shadow-xl border border-gray-200 rounded-2xl p-2 mt-2">
                        ...
                        <DropdownMenuItem asChild>
                          <Link to={createPageUrl("ForumHome")} className="cursor-pointer flex items-center gap-3 px-4 py-3 hover:bg-gray-50 rounded-lg">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
                <div className="md:col-span-1">
                  <p className="text-sm text-gray-400 leading-relaxed">Discover unique local experiences and accommodations.</p>
                </div>
                <div>
                  <h3 className="font-semibold text-white mb-4">Company</h3>
                  <ul className="space-y-2">
                    <li><Link to={createPageUrl("About")} className="hover:text-white transition-colors text-sm">About Us</Link></li>
                    <li><a href={`${createPageUrl("Home")}#services-section`} className="hover:text-white transition-colors text-sm">Services</a></li>
                    <li><Link to={createPageUrl("BecomeAHost")} className="hover:text-white transition-colors text-sm">Become a Host</Link></li>
                    <li><Link to={createPageUrl("Partner")} className="hover:text-white transition-colors text-sm">Partners</Link></li>
                  </ul>
                </div>
                <div>
                  <h3 className="font-semibold text-white mb-4">Support</h3>
                  <ul className="space-y-2">
                    <li><Link to={createPageUrl("CommunityGuidelines")} className="hover:text-white transition-colors text-sm">Community Guidelines</Link></li>
                  </ul>
                </div>
                <div>
                  <h3 className="font-semibold text-white mb-4">Legal</h3>
                  <ul className="space-y-2">
                    <li><Link to={createPageUrl("CookiePolicy")} className="hover:text-white transition-colors text-sm">Cookie Policy</Link></li>
                    <li><Link to={createPageUrl("PrivacyPolicy")} className="hover:text-white transition-colors text-sm">Privacy Policy</Link></li>
                  </ul>
                </div>
              </div>
              <div className="border-t border-gray-700 mt-8 pt-8 flex flex-col sm:flex-row justify-between items-center">
                ...
                <div className="flex items-center gap-4">
                  <a href="https://www.youtube.com/@sawaapp_io" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-all duration-300" aria-label="Visit SAWA on YouTube">
                    <Youtube className="w-6 h-6" />
                  </a>
                  <a href="https://www.instagram.com/travel_sawaapp/#" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-all duration-300" aria-label="Visit SAWA on Instagram">
