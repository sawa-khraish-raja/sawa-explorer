...
export default function CreateBooking() {
  ...
  const createBookingMutation = useMutation({
    mutationFn: (bookingPayload) => base44.entities.Booking.create(bookingPayload),
    onSuccess: async (newBooking) => {
      toast.success("Your booking request has been sent! You will be notified when a host accepts.", { duration: 5000 });
      ...
      try {
        ...
        const cityHosts = approvedHosts.filter(host => host.city === newBooking.city);
        ...
        if (hostEmails.length > 0) {
          ...
          console.log(`Booking request ${newBooking.id} sent to ${hostEmails.length} hosts in ${newBooking.city}.`);
        } else {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Card className="shadow-2xl border-2 border-white/50 rounded-3xl mt-8">
          <CardHeader className="bg-gray-50/50 rounded-t-3xl p-6 border-b">
            <CardTitle className="text-2xl font-bold text-gray-900">Finalize Your Request</CardTitle>
            <p className="text-gray-600">
              Confirm your details and add any special requests. We'll send this to available hosts in <span className="font-semibold text-[#330066]">{bookingData.city}</span>.
            </p>
          </CardHeader>
