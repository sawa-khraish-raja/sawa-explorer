...
export default function AdminMessages() {
  ...
  const filteredServiceConversations = serviceConversations.filter(convo =>
  ...
  const filteredAdventureConversations = adventureConversations.filter(convo =>
    convo.traveler_email?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    convo.host_emails?.some(email => email?.toLowerCase().includes(searchQuery.toLowerCase()))
  );
    ...
    <AdminLayout>
      <div className="space-y-4 sm:space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-500 mt-1">{conversations.length} conversations • {messages.length} messages</p>
        </div>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className="text-2xl font-bold text-purple-600">
                    {serviceConversations.length > 0 ? Math.round(totalServiceMessages / serviceConversations.length) : 0}
                  </p>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{adventureConversations.length}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4">
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    {adventureConversations.length > 0 ? Math.round(totalAdventureMessages / adventureConversations.length) : 0}
                  </p>
                </CardContent>
              </Card>
            </div>
