...
export default function AdminEvents() {
  ...
  const { data: lastSync } = useQuery({
    ...
    queryFn: async () => {
      const meta = await base44.entities.SystemMeta.filter({ key: 'events_last_sync' });
  ...
  const handleSyncEvents = async (city = null) => {
    ...
    try {
      ...
      if (response.data.ok) {
        toast.success(`✅ Events synced! ${response.data.results.map(r => `${r.city}: ${r.created}`).join(', ')}`);
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Past events are automatically filtered out. Use "Clean Expired" to remove them from database.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoadingEvents ? (
                ...
                <TableBody>
                  {events.map(event => (
                    <TableRow key={event.id}>
                      ...
                      <TableCell><Badge variant="outline">{event.city}</Badge></TableCell>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  Events are automatically synced every 24 hours using OpenAI. The system discovers real upcoming events, 
                  creates them in the database, and removes expired ones to keep your event listings fresh and relevant.
                </p>
                <div className="mt-3 p-3 bg-white/60 rounded-lg">
                  <p className="text-xs text-gray-500">
                    <strong>Pro tip:</strong> You can manually trigger sync anytime using the "Sync with AI" button above.
                  </p>
                </div>
              </div>
            </div>
