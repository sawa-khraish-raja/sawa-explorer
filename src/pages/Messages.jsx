...
export default function Messages() {
  ...
  // ✅ Load conversations
  const { data: rawConversations = [], isLoading: conversationsLoading, refetch: refetchConversations } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (isHost) {
        ...
        fetchedConversations = (fetchedConversations || []).filter(convo =>
          Array.isArray(convo.host_emails) && convo.host_emails.includes(user.email)
        );
      } else {
      ...
      return fetchedConversations.sort((a, b) => {
        const timeA = a.last_message_timestamp ? new Date(a.last_message_timestamp) : new Date(a.created_date);
        const timeB = b.last_message_timestamp ? new Date(b.last_message_timestamp) : new Date(b.created_date);
  ...
  const serviceConversations = conversations.filter(c => c.conversation_type === 'service' || !c.conversation_type);
  ...
  const handleConversationSelect = useCallback((conversationId) => {
    ...
    navigate(`/Messages?conversation_id=${conversationId}`, { replace: true });
  }, [navigate]);
  ...
  const currentConversations = selectedTab === 'services' ? serviceConversations : adventureConversations;
              ...
              {conversationsLoading ? (
                ...
                <div className="flex items-center justify-center h-full p-8">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600">Your {selectedTab} conversations will appear here</p>
                  </div>
                </div>
              )}
            </div>
