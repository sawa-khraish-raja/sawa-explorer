  ...
  useEffect(() => {
    ...
    if (perfData) {
      setPerformanceMetrics({
        domLoad: (perfData.domContentLoadedEventEnd - perfData.domContentLoadedEventStart).toFixed(2),
      ...
      items: [
        {
          ...
          recommendation: slides.length > 2 ? 'Remove extra videos immediately. More than 2 videos cause memory crashes on mobile.' : null
        },
        {
          ...
          message: slides.every(s => (s.display_duration || 10) === 10)
            ...
            : `❌ Inconsistent durations: ${slides.map(s => `${s.display_duration || 10}s`).join(', ')}`,
          severity: slides.every(s => (s.display_duration || 10) === 10) ? 'success' : 'warning',
          recommendation: !slides.every(s => (s.display_duration || 10) === 10) ? 'Update display_duration to 10 for all videos in database.' : null
        },
        {
          name: 'Video File Sizes',
          status: Object.values(videoSizes).every(size => parseFloat(size) < 5) ? 'pass' : 'warning',
          message: Object.keys(videoSizes).length > 0 
            ? `Sizes: ${Object.values(videoSizes).map(s => `${s}MB`).join(', ')}`
            : '⏳ Checking sizes...',
          severity: Object.values(videoSizes).every(size => parseFloat(size) < 5) ? 'success' : 'warning',
          recommendation: Object.values(videoSizes).some(size => parseFloat(size) >= 5) 
            ? 'Compress videos to < 5MB each. Use H.264 codec with lower bitrate.' : null
        },
        {
          name: 'Poster Images',
          ...
          recommendation: !slides.every(s => s.poster_image) ? 'Add poster_image for instant visual display while video loads.' : null
        }
      ]
    },
    {
      ...
      items: [
        {
          ...
          message: memoryInfo 
            ? `${memoryInfo.used} MB used / ${memoryInfo.limit} MB limit (${((parseFloat(memoryInfo.used) / parseFloat(memoryInfo.limit)) * 100).toFixed(1)}%)`
            : '⏳ Monitoring...',
          severity: memoryInfo && parseFloat(memoryInfo.used) < 150 ? 'success' : 'warning',
          recommendation: memoryInfo && parseFloat(memoryInfo.used) >= 150 
            ? 'High memory usage detected. Check for memory leaks.' : null
        },
        {
        ...
        {
          name: 'Page Load Time',
          status: performanceMetrics && parseFloat(performanceMetrics.pageLoad) < 3000 ? 'pass' : 'warning',
          message: performanceMetrics 
            ? `Page load: ${performanceMetrics.pageLoad}ms, DOM: ${performanceMetrics.domLoad}ms`
            : '⏳ Measuring...',
          severity: performanceMetrics && parseFloat(performanceMetrics.pageLoad) < 3000 ? 'success' : 'warning'
        }
      ]
    },
    {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Comprehensive analysis of homepage hero video performance, memory usage, and mobile optimization
          </p>
        </div>

        {/* Overall Score */}
        <Card className={`mb-6 border-4 ${
          criticalIssues > 0 ? 'border-red-500 bg-red-50' : 
          warnings > 2 ? 'border-yellow-500 bg-yellow-50' : 
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                ...
                <p className="text-gray-700">
                  ...
                  {criticalIssues === 0 && warnings > 2 && '⚠️ Multiple warnings - optimization needed'}
            ...
            <CardContent>
              <div className="space-y-4">
                {category.items.map((check, checkIndex) => (
                  <div key={checkIndex} className="border-l-4 pl-4" style={{
                    borderColor: 
                      check.severity === 'critical' ? '#ef4444' :
                      check.severity === 'warning' ? '#f59e0b' :
                      check.severity === 'success' ? '#10b981' : '#6b7280'
                  }}>
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          {check.severity === 'critical' && <XCircle className="w-5 h-5 text-red-500" />}
                          {check.severity === 'warning' && <AlertTriangle className="w-5 h-5 text-yellow-500" />}
                          {check.severity === 'success' && <CheckCircle className="w-5 h-5 text-green-500" />}
                      ...
                      <Badge variant={
                        ...
                        check.severity === 'warning' ? 'outline' :
                        'default'
                      }>
                        {check.status}
                      </Badge>
                ...
                {criticalIssues > 0 && (
                  <>
                    <li className="font-bold text-red-600">
                      🚨 CRITICAL: {allChecks.filter(c => c.severity === 'critical')[0]?.recommendation}
                    </li>
                  </>
                )}
                {allChecks
