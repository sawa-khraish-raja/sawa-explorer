...
import { MessageSquare, Loader2, ArrowLeft, MapPin, Calendar, Eye, CheckCircle, DollarSign } from 'lucide-react';
...
export default function OfficeMessages() {
  ...
  const { data: office } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (allOffices || []).find(o => 
        typeof o.email === 'string' && typeof user.email === 'string' && o.email.toLowerCase().trim() === user.email.toLowerCase().trim()
      );
    },
    enabled: !!user?.email,
  });
  ...
  const { data: hosts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const approvedRequests = (allRequests || []).filter(r => 
        r.office_name === office.name && r.status === 'approved'
      );
  ...
  const unreadMessages = (conversations || []).filter(c => 
    Array.isArray(c.unread_by_hosts) && c.unread_by_hosts.some(email => (hosts || []).map(h => h.email).includes(email))
  ).length;
        ...
        {conversations.length === 0 ? (
          <Card className="text-center py-16">
            <CardContent>
              ...
              <p className="text-gray-600">Conversations will appear when travelers contact your hosts</p>
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-3">
            {conversations.map(convo => {
              ...
              const isUnread = Array.isArray(convo.unread_by_hosts) && convo.unread_by_hosts.some(email => 
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {conversationHosts.length > 0 && (
                          <p className="text-xs text-gray-500 mb-1">
                            Your Host: {(conversationHosts || []).map(h => h.full_name).join(', ')}
                          </p>
                        )}
                        ...
                        {convo.last_message_timestamp && (
                          <p className="text-xs text-gray-400 mt-1">
                            {formatDistanceToNow(new Date(convo.last_message_timestamp), { addSuffix: true })}
                          </p>
                        )}
                      </div>
                ...
                <CardContent className="p-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-semibold text-gray-900">{selectedConversation.traveler_email}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600 mb-1">Your Host(s)</p>
                      <p className="font-semibold text-gray-900">
                        {(hosts || [])
                          .filter(h => Array.isArray(selectedConversation.host_emails) && selectedConversation.host_emails.includes(h.email))
                          .map(h => h.full_name)
                          .join(', ') || 'None'}
                      </p>
                    </div>
                ...
                <Card>
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <Badge className={
                          conversationBooking.status === 'confirmed' ? 'bg-green-100 text-green-800' :
                          conversationBooking.status === 'pending' ? 'bg-blue-100 text-blue-800' :
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">
                          {format(new Date(conversationBooking.start_date), 'MMM d')} - {format(new Date(conversationBooking.end_date), 'MMM d, yyyy')}
                        </span>
                      </div>
                      {conversationBooking.total_price && (
                        <div>
                          <span className="text-sm font-semibold text-green-600">${conversationBooking.total_price}</span>
                        </div>
                      )}
                    </div>
                  </CardContent>
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-3">
                      {conversationOffers.map(offer => {
                        ...
                        return (
                          <div key={offer.id} className="p-3 bg-purple-50 rounded-lg border border-purple-100">
                            <div className="flex items-center justify-between mb-2">
                              <span className="font-semibold text-gray-900">${offer.price?.toFixed(2) || offer.total_price?.toFixed(2) || '0.00'}</span>
                              <Badge className={
                  ...
                  {isTranslating ? (
                    ...
                    <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                      {processedMessages.map(msg => (
                        <MessageBubble
                          key={msg.id}
                          message={{ ...msg, content: msg.displayText, originalTextForDisplay: msg.originalText }}
                          currentUser={{ email: selectedConversation.traveler_email }}
