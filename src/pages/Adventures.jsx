...
import { Sparkles, MapPin, Compass, Calendar, X, Users, Star, ChevronRight, Search, Tag } from "lucide-react";
...
export default function Adventures() {
  ...
  const { data: adventures = [], isLoading: adventuresLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return allAdventures.filter((a) => {
        ...
        const adventureDate = new Date(a.date);
        adventureDate.setHours(0, 0, 0, 0);
        return isApproved && adventureDate >= now;
      });
    },
    staleTime: 600000,
    cacheTime: 900000
  ...
  const { data: adventurePosts = [], isLoading: postsLoading } = useQuery({
    ...
    queryFn: async () => {
      const allPosts = await base44.entities.AdventurePost.list('-created_date');
  ...
  const filteredAdventures = adventures.filter((adv) => {
    const cityMatch = selectedCity === 'All' || adv.city === selectedCity;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
      <section className="relative bg-gradient-to-br from-[#330066] via-[#7B2CBF] to-[#9933CC] py-16 sm:py-20 lg:py-24 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-16">
          <svg className="w-full h-full" viewBox="0 0 1440 120" fill="none" preserveAspectRatio="none">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        post.media_urls[0].type === 'video' ? (
                          ...
                          <img
                            ...
                            onError={(e) => {
                              ...
                              e.target.src = 'https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=400&q=80';
                            }}
                          />
                        )
                      )}
                      <div className="absolute top-3 left-3 flex gap-2">
                        <Badge className="bg-[#9933CC] text-white">
                          {post.category}
                        </Badge>
          ...
          {!isLoading && filteredAdventures.length === 0 && filteredPosts.length === 0 && (
            <div className="text-center py-16 bg-white rounded-2xl shadow-lg">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-3">No adventures or stories found</h3>
