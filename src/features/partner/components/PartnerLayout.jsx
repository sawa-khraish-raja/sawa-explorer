...
export default function PartnerLayout({ children }) {
  ...
  const translations = {
    en: {
      ...navItems.reduce((acc, item) => ({ ...acc, [item.name]: item.name }), {}),
      Settings: 'Settings',
      Logout: 'Logout',
    },
    ar: {
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} className='min-h-screen bg-gray-50 flex'>
      <aside
        className={`bg-white border-r border-gray-200 w-64 flex-shrink-0 flex flex-col fixed inset-y-0 z-50 md:relative md:translate-x-0 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        ...
        <div className='p-4 border-t mt-auto'>
          <Link
            to={createPageUrl('PartnerSettings')}
