...
export default function AdventuresSection() {
  ...
  const { data: adventures = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return allAdventures.filter(adv => adv.status === 'upcoming' && new Date(adv.date) > new Date());
    },
    staleTime: 0,
    cacheTime: 0,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {adventures.map((adventure) => {
        ...
        return (
          <Card key={adventure.id} className="overflow-hidden hover:shadow-xl transition-shadow duration-300">
