...
function AcceptedHostItem({ hostEmail, booking }) {
    ...
    const { data: host, isLoading: hostLoading, isError } = useQuery({
    ...
    const acceptOfferMutation = useMutation({
        mutationFn: async ({ offer }) => {
            ...
            const updatedOffer = await base44.entities.Offer.update(offer.id, { status: 'accepted' });
            ...
            return { acceptedOffer: updatedOffer, updatedBooking, conversation_id: conversation?.id };
        },
        onSuccess: async ({ acceptedOffer, updatedBooking, conversation_id }) => {
            queryClient.invalidateQueries({ queryKey: ['bookingConversations', booking.id] });
            queryClient.invalidateQueries({ queryKey: ['offersForBooking', booking.id] });
            ...
            try {
                ...
                const allConversations = await base44.entities.Conversation.filter({ booking_id: bookingId });
                const allOffers = await base44.entities.Offer.filter({ booking_id: bookingId });

                const conversationsToDelete = allConversations.filter(c => 
                    c.host_emails && !c.host_emails.includes(winningHostEmail)
                );

                for (const convo of conversationsToDelete) {
                    console.log(`Deleting conversation ${convo.id} for losing host...`);
                    const messagesToDelete = await base44.entities.Message.filter({ conversation_id: convo.id });
                ...
                const offersToDelete = allOffers.filter(o => 
                    o.host_email !== winningHostEmail && o.status === 'pending'
                );
            ...
            <div className="bg-gradient-to-br from-purple-50 to-blue-50 rounded-2xl p-4 shadow-sm border-2 border-[var(--brand-bg-accent)]">
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="font-bold text-base sm:text-lg text-[var(--brand-primary)] truncate">Host</p>
            ...
            <div className="flex items-center gap-4 mb-4">
                <Avatar className="w-16 h-16 sm:w-20 sm:h-20 border-3 border-[var(--brand-primary)] flex-shrink-0">
                    ...
                    <AvatarFallback className="bg-gradient-to-br from-[var(--brand-primary)] to-[var(--brand-secondary)] text-white text-xl sm:text-2xl font-bold">
                        {host.full_name ? host.full_name.charAt(0).toUpperCase() : host.email.charAt(0).toUpperCase()}
                    </AvatarFallback>
                </Avatar>
                <div className="flex-1 min-w-0">
                    <p className="font-bold text-base sm:text-xl text-gray-900 mb-1 truncate">{host.full_name || host.email}</p>
                    {host.city && (
                        <p className="text-xs sm:text-sm text-gray-600 mb-1">{host.city}</p>
                    )}
                    {host.rating && (
                        <div className="flex items-center gap-1">
                            <Star className="w-4 h-4 fill-amber-400 text-amber-400" />
                            <span className="text-sm sm:text-base font-semibold text-gray-900">{host.rating.toFixed(1)}</span>
                        </div>
                    )}
                    {!hostConversation && (
                        <p className="text-xs text-red-500 mt-1">No conversation found</p>
                    )}
                </div>
            </div>
            {hasPendingOffer ? (
                ...
                >
                    {acceptOfferMutation.isPending ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <Star className="w-5 h-5 mr-2" />}
...
export default function BookingOffersView({ booking, onBack }) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-purple-50 px-4 py-6 sm:p-6">
            ...
            <div className="mb-6">
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[var(--brand-primary)] mb-2">Choose Your Host</h1>
                <p className="text-sm sm:text-base text-gray-600">
                    {hasAcceptedHosts 
                        ? `${acceptedHosts.length} host${acceptedHosts.length > 1 ? 's' : ''} accepted your booking request for ${booking.city}` 
            ...
            <div className="space-y-4">
                {hasAcceptedHosts ? (
                            ...
                            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-4 border-2 border-green-200 shadow-sm">
                                <div className="flex items-center gap-3">
                                    ...
                                    <p className="text-sm sm:text-base text-green-800 font-semibold">
                                        {pendingOffers.length} price {pendingOffers.length === 1 ? 'offer' : 'offers'} received! Check your messages to see details.
                                    </p>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="bg-white rounded-2xl p-6 sm:p-8 text-center shadow-lg border-2 border-[var(--brand-bg-accent)]">
                        ...
                        <h3 className="text-lg sm:text-xl font-bold text-[var(--brand-primary)] mb-2">Waiting for Host Approval</h3>
                        <p className="text-sm sm:text-base text-gray-600 mb-4">
                            Your booking request has been sent to all local hosts in {booking.city}. They will review and accept your request soon.
                        </p>
