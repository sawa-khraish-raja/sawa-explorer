...
import { Calendar, Users, MapPin, MessageSquare, Loader2, ChevronRight, Info, CheckCircle, XCircle, Clock, Sparkles } from 'lucide-react';
...
export default function AdventureBookingCard({ booking }) {
  ...
  const { data: conversation } = useQuery({
    ...
    queryFn: async () => {
      const convos = await base44.entities.Conversation.filter({ booking_id: booking.id });
      return convos[0];
    },
    enabled: !!booking.id,
  });
  ...
  const { data: host } = useQuery({
    ...
    queryFn: async () => {
      const users = await base44.entities.User.filter({ email: adventure.host_email });
      return users[0];
    },
        ...
        <div className="flex-1 min-w-0">
          ...
          {booking.status === 'confirmed' && host && (
            <div className="flex items-center gap-2 mb-2 p-2 bg-white/60 rounded-lg border border-purple-200">
              {host.profile_photo ? (
                <img src={host.profile_photo} alt={host.full_name} className="w-8 h-8 rounded-full object-cover border-2 border-purple-300" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-purple-900 truncate">Your Host: {host.full_name}</p>
                {host.rating && (
