...
export default function AdventureForm({ adventure, hostType, onSubmit, onCancel, isSubmitting }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.title || !formData.description || !formData.city || !formData.date || !formData.host_price) {
    ...
    const submissionData = {
      ...
      traveler_total_price: commissionBreakdown.travelerPays,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          <Label htmlFor="date">Date & Time <span className="text-red-500 ml-1">*</span></Label>
          <Input
            id="date"
            name="date"
            type="datetime-local"
        ...
        <div>
          <Label htmlFor="duration_hours">Duration (hours) <span className="text-red-500 ml-1">*</span></Label>
          <Input
            id="duration_hours"
            name="duration_hours"
            type="number"
        ...
        <div>
          <Label htmlFor="max_participants">Max Participants <span className="text-red-500 ml-1">*</span></Label>
          <Input
            id="max_participants"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <Label htmlFor="host_price">
              {hostType === 'admin' ? 'Traveler Price (USD)' : 
               hostType === 'office' ? 'Office Receives (USD)' :
               'You Receive (USD)'}
              ...
              <div className="space-y-2">
                {commissionBreakdown.breakdown.map((item, idx) => (
                  <div key={idx} className="flex justify-between text-sm">
                    <span className={
                      item.type === 'host' ? 'font-semibold text-gray-900' :
                      item.type === 'sawa' ? 'text-purple-700' :
                      item.type === 'office' ? 'text-blue-700' :
                      'text-gray-600'
                    }>
                      {item.label}:
                    </span>
                ...
                <div className="pt-2 border-t-2 border-purple-300 flex justify-between text-base font-bold">
                  ...
                  <span className="text-purple-900">${commissionBreakdown.travelerPays.toFixed(2)}</span>
                </div>
              </div>
      ...
      {/* Meeting Point */}
      <div>
        <Label htmlFor="meeting_point">Meeting Point <span className="text-red-500 ml-1">*</span></Label>
        <Input
          id="meeting_point"
          name="meeting_point"
      ...
      <div className="flex justify-end gap-3 pt-4 border-t">
        ...
        <Button type="submit" disabled={isSubmitting} className="bg-[#7B2CBF] hover:bg-[#6A1FA0]">
          {isSubmitting ? (
            ...
            adventure ? 'Update Adventure' : 'Create Adventure'
          )}
        </Button>
      </div>
    </form>
