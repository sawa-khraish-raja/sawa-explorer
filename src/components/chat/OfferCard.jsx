...
import { Check, X, DollarSign, Calendar, Package, AlertCircle, Loader2, CheckCircle2 } from 'lucide-react';
import { format } from 'date-fns';
import { base44 } from '@/api/base44Client';
import { useMutation, useQueryClient, useQuery } from '@tanstack/react-query';
import { showSuccess, showError } from '../utils/notifications';

export default function OfferCard({ offer, booking, onAccept, onDecline, viewerType = 'traveler' }) {
      ...
      <CardContent className="space-y-3">
        ...
        {offer.expiry_date && isPending && (
          <div className="flex items-center gap-2 text-xs text-gray-600 p-2 bg-orange-50 rounded-lg border border-orange-200">
            ...
            <span>
              Expires on {format(new Date(offer.expiry_date), 'MMM dd, yyyy')}
            </span>
          </div>
        )}
