...
import { Copy, Zap, CheckCircle2, AlertCircle, Loader2, ChevronRight, Clock, X, XCircle, Check, DollarSign, AlertTriangle } from 'lucide-react';
...
export default function MessageBubble({
  ...
  const needsTranslation = !isUser && 
                           normalizedSourceLang && 
                           normalizedSourceLang !== normalizedDisplayLang;
  ...
  const displayText = useMemo(() => {
    ...
    return message.original_text;
  }, [message, normalizedDisplayLang, needsTranslation, showOriginal, translatedTextState, cachedTranslation, isUser]);
  ...
  }, [message.id, normalizedDisplayLang, needsTranslation, isUser, cachedTranslation, translatedTextState, showOriginal, message.original_text]);
  ...
  const renderOfferCard = (offerData) => {
    ...
    const getBlockingMessage = () => {
      ...
      if (isServiceOffer && acceptedServiceOffer && acceptedServiceOffer.id !== offerData.id) {
        return translatedText('Another service offer was already accepted', 'تم قبول عرض خدمات آخر');
      }
    ...
    const shouldShowCheckout = isAccepted && isServiceOffer && !isHostInConversation && !isBookingCancelled;
    ...
    return (
      <Card className={cn(
        ...
        isPending && !blockingMessage && "bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-300",
        ...
        <CardContent className="space-y-3">
          ...
          {offerData.price_breakdown && isServiceOffer && (
            <div className="p-3 bg-gray-50 rounded-lg border border-gray-200 text-xs space-y-1">
              ...
              {offerData.price_breakdown.sawa_fee > 0 && (
                <div className="flex justify-between text-gray-600">
                  <span>{translatedText('SAWA Fee', 'رسوم SAWA')} ({offerData.price_breakdown.sawa_percent}%)</span>
                  <span className="font-semibold">${offerData.price_breakdown.sawa_fee}</span>
                </div>
              )}
              {offerData.price_breakdown.office_fee > 0 && (
                <div className="flex justify-between text-gray-600">
                  <span>{translatedText('Office Fee', 'رسوم المكتب')} ({offerData.price_breakdown.office_percent}%)</span>
          ...
          {offerData.rental_details && (
            <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <p className="text-xs text-gray-700 whitespace-pre-wrap">{offerData.rental_details}</p>
            </div>
          )}
            ...
            <div className="mt-4 space-y-2">
              {canAcceptThisOffer ? (
                <div className="flex gap-2">
                  <Button
                    onClick={async () => {
                      console.log('✅ [MessageBubble] Accept button clicked for offer:', offerData.id);
                  ...
                  >
                    {isAcceptingOffer ? (
                      <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> {translatedText('Accepting...', 'جاري القبول...')}</>
                    ) : (
                      <><CheckCircle2 className="w-4 h-4 mr-2" /> {translatedText('Accept Offer', 'قبول العرض')}</>
                    )}
                  </Button>
