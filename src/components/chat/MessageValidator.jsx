    ...
    static validate(message) {
        ...
        if (violationPatterns.phone.test(message)) {
            ...
            if (phoneMatches.some(match => match.replace(/[^0-9]/g, '').length >= 7)) {
                 violations.push({ type: 'phone', message: violationMessages.phone.en, messageAr: violationMessages.phone.ar });
            }
        }
        
        // Test for emails
        if (violationPatterns.email.test(message)) {
            violations.push({ type: 'email', message: violationMessages.email.en, messageAr: violationMessages.email.ar });
        }
        
        // Test for links
        if (violationPatterns.link.test(message)) {
            violations.push({ type: 'link', message: violationMessages.link.en, messageAr: violationMessages.link.ar });
        }
