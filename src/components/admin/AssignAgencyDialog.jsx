        ...
        <div className="space-y-4 py-4">
          ...
          <div className="bg-gray-50 p-3 rounded-lg">
            ...
            <div className="mt-2 text-xs text-gray-600">
              Current: {host?.host_type === 'agency' ? 
                `Agency Host (${agencies.find(a => a.id === host.agency_id)?.name || 'Unknown'})` : 
                'Freelancer Host'}
            </div>
          </div>
          ...
          <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-200">
            ...
            {selectedAgencyId && selectedAgency ? (
              <div className="space-y-1 text-sm text-indigo-800">
                <p>• Host Type: <Badge className="ml-2 bg-indigo-600 text-white">Agency</Badge></p>
                ...
                <div className="mt-2 pt-2 border-t border-indigo-200 text-xs text-indigo-600">
                  <p><strong>Example:</strong> If host sets service at $100:</p>
                  <p>• SAWA gets: ${(100 * (selectedAgency.commission_sawa_default || 28) / 100).toFixed(2)}</p>
                  <p>• Office gets: ${(100 * (selectedAgency.commission_office_default || 7) / 100).toFixed(2)}</p>
                  <p>• Traveler pays: ${(100 + (100 * (selectedAgency.commission_sawa_default || 28) / 100) + (100 * (selectedAgency.commission_office_default || 7) / 100)).toFixed(2)}</p>
                </div>
              </div>
            ) : (
              <div className="space-y-1 text-sm text-indigo-800">
                <p>• Host Type: <Badge className="ml-2 bg-purple-600 text-white">Freelancer</Badge></p>
