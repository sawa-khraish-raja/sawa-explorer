    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-full sm:max-w-4xl max-h-[90vh] overflow-y-auto bg-gradient-to-br from-[#E6E6FF] to-white">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="bg-gradient-to-br from-[#330066] to-[#9933CC] text-white border-0">
              <CardContent className="p-4 sm:p-6">
                ...
                <p className="text-xs text-white/70 mt-2">{revenueData.confirmedBookingsCount} confirmed booking{revenueData.confirmedBookingsCount !== 1 ? 's' : ''}</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-[#9933CC] to-[#AD5CD6] text-white border-0">
              <CardContent className="p-4 sm:p-6">
                ...
                <p className="text-2xl sm:text-3xl font-bold">${revenueData.officeHost.toFixed(2)}</p>
                <p className="text-xs text-white/70 mt-2">{revenueData.officeBookingsCount} booking{revenueData.officeBookingsCount !== 1 ? 's' : ''}</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-[#AD5CD6] to-[#CE9DE7] text-white border-0">
              <CardContent className="p-4 sm:p-6">
                ...
                <p className="text-2xl sm:text-3xl font-bold">${revenueData.independent.toFixed(2)}</p>
                <p className="text-xs text-white/70 mt-2">{revenueData.independentBookingsCount} booking{revenueData.independentBookingsCount !== 1 ? 's' : ''}</p>
              </CardContent>
            </Card>
          </div>
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <InsightCard
                  ...
                  value={`$${revenueData.confirmedBookingsCount > 0 ? (revenueData.total / revenueData.confirmedBookingsCount).toFixed(2) : '0.00'}`}
                ...
                <InsightCard
                  icon={TrendingUp}
                  title="Highest"
                  value={`$${revenueData.breakdown.length > 0 ? Math.max(...revenueData.breakdown.map((b) => b.commission)).toFixed(2) : '0.00'}`}
                ...
                <InsightCard
                  icon={Building2}
                  title="Office Share"
                  value={`${revenueData.total > 0 ? (revenueData.officeHost / revenueData.total * 100).toFixed(1) : '0.0'}%`}
            ...
            <CardContent className="p-6">
              <div className="space-y-3">
                {(() => {
                  ...
                  const sortedCityRevenue = Object.entries(cityRevenue).sort(([, a], [, b]) => b.total - a.total);

                  return sortedCityRevenue.length > 0 ? sortedCityRevenue.map(([city, data]) => (
                    <CityRevenueRow key={city} city={city} data={data} totalRevenue={revenueData.total} />
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t">
                  <AverageCard
                    ...
                    value={`$${revenueData.officeBookingsCount > 0 ? (revenueData.officeHost / revenueData.officeBookingsCount).toFixed(2) : '0.00'}`}
                    color="bg-purple-50 text-[#9933CC]"
                  />
                  <AverageCard
                    title="Avg per Independent Booking"
                    value={`$${revenueData.independentBookingsCount > 0 ? (revenueData.independent / revenueData.independentBookingsCount).toFixed(2) : '0.00'}`}
          ...
          <Card className="bg-white border-2 border-[#CCCCFF]">
            <CardHeader className="bg-gradient-to-r from-[#E6E6FF] to-white border-b">
              <CardTitle className="text-base sm:text-lg text-[#330066]">📊 Booking-by-Booking Breakdown</CardTitle>
            </CardHeader>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50 border-b">
                      <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-[10px] sm:text-xs font-semibold text-gray-600">Booking ID</th>
                      <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-[10px] sm:text-xs font-semibold text-gray-600">City</th>
                      <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-[10px] sm:text-xs font-semibold text-gray-600">Host Type</th>
                      <th className="px-3 py-2 sm:px-4 sm:py-3 text-left text-[10px] sm:text-xs font-semibold text-gray-600">Rate</th>
                      <th className="px-3 py-2 sm:px-4 sm:py-3 text-right text-[10px] sm:text-xs font-semibold text-gray-600">Commission</th>
                    </tr>
                  </thead>
                  <tbody>
                    {revenueData.breakdown.map((item, idx) => (
                      <tr key={idx} className="border-b hover:bg-purple-50 transition-colors">
                        <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm font-mono text-gray-600">{item.booking_id.slice(0, 8)}</td>
                        <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm text-gray-900">{item.city || 'N/A'}</td>
                        <td className="px-3 py-2 sm:px-4 sm:py-3">
                          <Badge className={item.host_type === 'Office' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'}>
                            {item.host_type}
                          </Badge>
                        </td>
                        <td className="px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm font-semibold text-[#9933CC]">{item.rate}</td>
                        <td className="px-3 py-2 sm:px-4 sm:py-3 text-right text-xs sm:text-sm font-bold text-green-600">${item.commission.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr className="bg-gradient-to-r from-[#E6E6FF] to-white font-bold">
                      <td colSpan="4" className="px-3 py-3 sm:px-4 sm:py-4 text-right text-[#330066]">Total SAWA Revenue:</td>
                      <td className="px-3 py-3 sm:px-4 sm:py-4 text-right text-lg sm:text-xl text-[#330066]">${revenueData.total.toFixed(2)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
...
const InsightCard = React.memo(({ icon: Icon, title, value, subtitle, color }) => (
  <div className="text-center p-3 bg-white rounded-lg shadow-sm">
    <div className={`w-7 h-7 sm:w-8 sm:h-8 ${color} rounded-lg flex items-center justify-center mx-auto mb-2`}>
  ...
  <div className="flex items-center justify-between p-2 sm:p-3 bg-purple-50 rounded-lg hover:bg-purple-100 transition-colors">
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-xs text-gray-600">{data.count} booking{data.count !== 1 ? 's' : ''}</p>
      </div>
    </div>
