...
export default function AdminLayout({ children, currentPage = '' }) {
  ...
  useEffect(() => {
    if (user) {
      const hasAnyAdminAccess = hasFullAccess || (user.admin_allowed_pages && user.admin_allowed_pages.length > 0);
  ...
  const menuItems = [
    ...
    { id: 'adventures', name: 'Adventures', icon: Zap, path: 'AdminAdventures', count: notificationCounts.pendingAdventures },
    { id: 'cities', name: 'Cities', icon: MapPin, path: 'AdminCities' },
    { id: 'offices', name: 'Offices', icon: Building2, path: 'AdminOffices' },
    
    // ✅ RESTORED + NEW
    { id: 'customer_support', name: 'Customer Support', icon: HelpCircle, path: 'CustomerSupport', badge: 'New' },
    { id: 'host_requests', name: 'Host Requests', icon: UserCheck, path: 'AdminHostRequests', count: notificationCounts.hostRequests },
    { id: 'partner_requests', name: 'Partner Requests', icon: Briefcase, path: 'AdminPartnerRequests', count: notificationCounts.partnerRequests },
    { id: 'cancellations', name: 'Cancellations', icon: XCircle, path: 'AdminCancellations', count: notificationCounts.pendingCancellations },
    { id: 'messages', name: 'Messages', icon: MessageSquare, path: 'AdminMessages' },
    { id: 'events', name: 'Events', icon: Calendar, path: 'AdminEvents' },
    { id: 'hero_slides', name: 'Hero Slides', icon: Image, path: 'AdminHeroSlides' },
    { id: 'forum', name: 'Forum Moderation', icon: MessageSquare, path: 'AdminForumModeration' },
    { id: 'adventure_posts', name: 'Adventure Posts', icon: Image, path: 'AdminAdventurePosts' },
    { id: 'ai_monitoring', name: 'AI Monitoring', icon: Activity, path: 'AdminAIMonitoring' },
    { id: 'agencies', name: 'Agencies', icon: Building2, path: 'AdminAgencies' },
    { id: 'audit_logs', name: 'Audit Logs', icon: FileText, path: 'AdminAuditLogs' },
    
    { id: 'broadcast', name: 'Broadcast', icon: Bell, path: 'AdminBroadcast', badge: 'New' },
    { id: 'analytics', name: 'Analytics', icon: TrendingUp, path: 'AdminAnalytics' },
                ...
                {totalNotifications > 0 && (
                  <div className="mt-2 bg-red-500/20 border border-red-400/30 rounded-lg px-3 py-2 text-center">
                    <p className="text-red-200 text-xs font-semibold">
                      {totalNotifications} New {totalNotifications === 1 ? 'Notification' : 'Notifications'}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="p-4 border-t border-white/10">
              ...
              {!hasFullAccess && user.admin_allowed_pages && (
                <div className="text-xs text-white/50 mt-1">
                  {user.admin_allowed_pages.length} page{user.admin_allowed_pages.length !== 1 ? 's' : ''} access
                </div>
              )}
            </div>
          )}
