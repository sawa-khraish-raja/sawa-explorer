...
export default function BookingForm({ 
  ...
  const createBookingMutation = useMutation({
    mutationFn: async (bookingData) => {
      // ✅ Track booking start
      trackBookingStart(city, { start: startDate, end: endDate }, { adults, children }, selectedServices);
    ...
    onSuccess: (booking) => {
      showSuccess(
        ...
        language === 'ar' 
          ? 'سيتواصل معك المضيفون قريباً' 
          : 'Hosts will contact you soon'
      );
    ...
    onError: (error) => {
      showError(
        language === 'ar' ? 'فشل إرسال الطلب' : 'Failed to send request',
        error.message
      );
    }
  });
