...
import {
  Calendar, MapPin, Users, DollarSign, User, Mail, Phone,
  MessageSquare, Package, FileText, Clock, CheckCircle2,
  XCircle, AlertTriangle, Star, Sparkles, Building2,
  Copy, History, Receipt, Info, Briefcase
} from 'lucide-react';
...
export default function BookingDetailsModal({ booking, open, onOpenChange, viewerType = 'traveler' }) {
  ...
  const declinedOffers = useMemo(() =>
    offers.filter(o => o.status === 'declined'),
    [offers]
  );
    ...
    offers.forEach(offer => {
      if (!hostMap.has(offer.host_email)) {
        ...
        const conversation = conversations.find(c =>
          c.booking_id === booking?.id && c.host_emails && c.host_emails.includes(offer.host_email)
        );
  ...
  const hostsWithConversations = useMemo(() => {
    ...
    if (isAdventureBooking) {
      if (host) {
        const conversation = conversations.find(c =>
          c.booking_id === booking?.id && c.host_emails && c.host_emails.includes(host.email)
        );
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-3xl lg:max-w-5xl max-h-[90vh] overflow-hidden p-0">
        ...
        <DialogHeader className="p-3 sm:p-6 pb-3 sm:pb-4 border-b border-gray-100 bg-gradient-to-r from-purple-50 to-white">
          <div className="flex items-start justify-between gap-2 sm:gap-4">
            ...
            <div className={cn("px-2 sm:px-4 py-1 sm:py-2 rounded-full border-2 flex items-center gap-1 sm:gap-2 flex-shrink-0", statusConfig.bg, statusConfig.border)}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                      ...
                      <CardContent className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-2 sm:gap-3 text-xs sm:text-sm">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold truncate ml-1">{format(new Date(adventure.date), 'MMM d')}</span>
                          </div>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold">{booking.number_of_adults} {booking.number_of_adults === 1 ? 'Guest' : 'Guests'}</span>
                          </div>
                        </div>
                        ...
                        {adventure.description && (
                          <div className="pt-2 sm:pt-3 border-t">
                            <p className="text-xs sm:text-sm text-gray-700 line-clamp-3">{adventure.description}</p>
                          </div>
                        )}
                      ...
                      <CardContent className="p-3 sm:p-4">
                        <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className="font-semibold">${(adventure.traveler_total_price || 0).toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          <div className="flex justify-between pt-2 border-t border-gray-200 text-sm sm:text-base font-bold">
                            <span>Total:</span>
                            <span className="text-green-600">${(booking.total_price || 0).toFixed(2)}</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                      ...
                      <Card className="bg-gradient-to-r from-purple-50 to-purple-100/50 border-2 border-purple-200">
                        ...
                        <CardContent className="p-3 sm:p-4">
                          <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                            {host.profile_photo ? (
                              <img src={host.profile_photo} alt={getUserDisplayName(host)} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover border-2 border-purple-300" />
                            ) : (
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="font-semibold text-sm sm:text-base truncate">{getUserDisplayName(host)}</p>
                              {host.rating && (
                          ...
                          {(() => {
                            const hostConversation = conversations.find(c =>
                    ...
                    <Card>
                      ...
                      <CardContent className="p-3 sm:p-4 space-y-2 sm:space-y-3 text-xs sm:text-sm">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-semibold truncate ml-1">{normalizeText(booking.city)}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-semibold">{format(new Date(booking.start_date), 'MMM d, yyyy')}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-semibold">{format(new Date(booking.end_date), 'MMM d, yyyy')}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-semibold">
                            ...
                            {booking.number_of_children > 0 && ` + ${booking.number_of_children} ${booking.number_of_children === 1 ? 'Child' : 'Children'}`}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t">
                          ...
                          <span className="font-semibold text-[10px] sm:text-xs">{format(new Date(booking.created_date), 'MMM d, yyyy HH:mm')}</span>
                        </div>
                      </CardContent>
                    </Card>
                    ...
                    <Card>
                      ...
                      <CardContent className="p-3 sm:p-4">
                        {booking.selected_services && booking.selected_services.length > 0 ? (
                          ...
                          <p className="text-xs sm:text-sm text-gray-500 italic">No specific services requested</p>
                        )}
                      </CardContent>
                    </Card>
                          ...
                          <div className="space-y-3">
                            {hostsWithConversations.map(({ email, user, offer, conversation }) => {
                              const getOfferStatusBadge = () => {
                                if (!offer) {
                                  return <Badge className="bg-blue-500 text-white text-[8px] sm:text-[10px]">📩 No Offer Yet</Badge>;
                                }

                                switch (offer.status) {
                                  case 'accepted':
                                    return <Badge className="bg-green-600 text-white text-[8px] sm:text-[10px]">✓ Offer Accepted</Badge>;
                                  case 'pending':
                                    return <Badge className="bg-orange-500 text-white text-[8px] sm:text-[10px]">⏳ Offer Pending</Badge>;
                                  case 'declined':
                                    return <Badge className="bg-gray-400 text-white text-[8px] sm:text-[10px]">✗ Offer Declined</Badge>;
                                  default:
                                    return <Badge className="bg-blue-500 text-white text-[8px] sm:text-[10px]">{offer.status}</Badge>;
                                }
                              };
                                  ...
                                  <div className="flex items-center gap-3 flex-1 min-w-0">
                                    {user?.profile_photo ? (
                                      ...
                                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center text-white font-bold text-sm">
                                        {getUserDisplayName(user)?.charAt(0) || email.charAt(0).toUpperCase()}
                                      </div>
                                    )}
                          ...
                          {acceptedOffer?.price_breakdown ? (
                            <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                              <div className="flex justify-between">
                                ...
                                <span className="font-semibold">${acceptedOffer.price_breakdown.base_price?.toFixed(2)}</span>
                              </div>
                              <div className="flex justify-between text-[10px] sm:text-xs text-gray-500">
                                <span>+ SAWA Commission ({acceptedOffer.price_breakdown.sawa_percent}%):</span>
                                <span>${acceptedOffer.price_breakdown.sawa_fee?.toFixed(2)}</span>
                              </div>
                              {acceptedOffer.price_breakdown.office_percent > 0 && (
                                <div className="flex justify-between text-[10px] sm:text-xs text-gray-500">
                                  <span>+ Office Commission ({acceptedOffer.price_breakdown.office_percent}%):</span>
                                  <span>${acceptedOffer.price_breakdown.office_fee?.toFixed(2)}</span>
                                </div>
                              )}
                              <div className="flex justify-between pt-2 border-t border-gray-200 text-sm sm:text-base font-bold">
                                <span>Total:</span>
                            ...
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-xl sm:text-2xl font-bold text-green-600">${booking.total_price.toFixed(2)}</span>
                            </div>
                          )}
                        </CardContent>
                      </Card>
                ...
                {booking.notes && (
                  <Card className="md:col-span-2">
                    ...
                    <CardContent className="p-3 sm:p-4">
                      <p className="text-xs sm:text-sm text-gray-700 italic">"{normalizeText(booking.notes)}"</p>
                    </CardContent>
                  </Card>
                )}
                      ...
                      {cancellationRequest && (
                        <>
                          <div className="flex justify-between">
                            ...
                            <span className="font-semibold capitalize">{cancellationRequest.requester_type}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Reason:</span>
                            <span className="font-semibold">{cancellationRequest.reason_category?.replace('_', ' ')}</span>
                          </div>
                          {cancellationRequest.calculated_refund && (
                            <div className="flex justify-between pt-2 border-t">
                              <span className="text-gray-600">Refund Amount:</span>
                              <span className="font-bold text-green-600">${cancellationRequest.calculated_refund.toFixed(2)}</span>
                            </div>
                          )}
                        </>
                      )}
                ...
                {offers.length === 0 ? (
                  <Card className="text-center py-8">
                    <CardContent className="p-3 sm:p-4">
                      ...
                      <p className="text-xs sm:text-sm text-gray-500 mt-1">Waiting for hosts to submit offers</p>
                    </CardContent>
                  </Card>
                ) : (
                  <div className="space-y-3">
                    ...
                    {acceptedOffer && (
                      <Card className="bg-green-50 border-2 border-green-200">
                        ...
                        <CardContent className="p-3 sm:p-4 space-y-3">
                          <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                            {host?.profile_photo ? (
                              <img src={host.profile_photo} alt={getUserDisplayName(host)} className="w-10 h-10 rounded-full object-cover border-2 border-green-300" />
                            ) : (
                            ...
                            <div>
                              <p className="font-semibold text-gray-900 text-sm">{getUserDisplayName(host)}</p>
                          ...
                          {acceptedOffer.inclusions && (
                            <div className="pt-2 border-t border-green-200">
                              <p className="text-xs font-semibold text-gray-700 mb-1">Inclusions:</p>
                          ...
                          {acceptedOffer.message && (
                            <div className="pt-2 border-t border-green-200">
                              ...
                              <p className="text-xs text-gray-600 italic">"{acceptedOffer.message}"</p>
                            </div>
                          )}
                        </CardContent>
                      </Card>
                    ...
                    {pendingOffers.length > 0 && (
                      <>
                        <div className="flex items-center gap-2 mt-4">
                          ...
                          <h4 className="font-semibold text-sm">Pending Offers ({pendingOffers.length})</h4>
                        </div>
                              ...
                              <div className="flex justify-between items-start">
                                <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-2">
                                    <p className="font-semibold text-gray-900 text-sm">{offer.host_email}</p>
                                    <Badge className="bg-orange-500 text-white text-[10px]">Pending</Badge>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-2 gap-2 text-sm">
                                    ...
                                    <div>
                                      ...
                                      <p className="font-semibold capitalize text-xs">{offer.offer_type}</p>
                                    </div>
                                  </div>
                    ...
                    {declinedOffers.length > 0 && (
                      <>
                        <div className="flex items-center gap-2 mt-4">
                          ...
                          <h4 className="font-semibold text-sm text-gray-600">Declined Offers ({declinedOffers.length})</h4>
                        </div>
                        {declinedOffers.map(offer => (
                          <Card key={offer.id} className="border-gray-200 bg-gray-50 opacity-60">
                            <CardContent className="p-3 sm:p-4">
                              <div className="flex justify-between items-center">
                                <div>
                                  <p className="font-semibold text-gray-600 text-sm">{offer.host_email}</p>
                                  <p className="text-xs text-gray-500">
                                    ${(offer.price_total || offer.price)?.toFixed(2)} • {offer.offer_type}
                                  </p>
                                </div>
                                <Badge variant="outline" className="text-gray-500 border-gray-300 text-[10px]">Declined</Badge>
                              </div>
                            </CardContent>
                          </Card>
                        ))}
                  ...
                  <Card>
                    ...
                    <CardContent className="p-3 sm:p-4 space-y-2 sm:space-y-3">
                      {traveler ? (
                        <>
                          <div className="flex items-center gap-2 sm:gap-3">
                            {traveler.profile_photo ? (
                              <img src={traveler.profile_photo} alt={getUserDisplayName(traveler)} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover border-2 border-purple-200" />
                            ) : (
                              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center text-white font-bold text-sm sm:text-base">
                            ...
                            <div>
                              <p className="font-semibold text-sm sm:text-base">{getUserDisplayName(traveler)}</p>
                          ...
                          <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                            <div className="flex items-center gap-2">
                              ...
                              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => copyToClipboard(traveler.email, 'Email')}>
                        ...
                        <p className="text-xs sm:text-sm text-gray-500">No traveler information available</p>
                      )}
                    </CardContent>
                  </Card>
                    ...
                    <CardContent className="p-3 sm:p-4 space-y-2 sm:space-y-3">
                      {host ? (
                        <>
                          <div className="flex items-center gap-2 sm:gap-3">
                            {host.profile_photo ? (
                              <img src={host.profile_photo} alt={getUserDisplayName(host)} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover border-2 border-green-200" />
                            ) : (
                            ...
                            <div>
                              <p className="font-semibold text-sm sm:text-base">{getUserDisplayName(host)}</p>
                              {host.rating && (
                                <div className="flex items-center gap-1 text-[10px] sm:text-xs text-amber-600">
                                  <Star className="w-2.5 h-2.5 sm:w-3 sm:h-3 fill-current" />
                                  {host.rating.toFixed(1)}
                          ...
                          <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                            <div className="flex items-center gap-2">
                              ...
                              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => copyToClipboard(host.email, 'Email')}>
                                <Copy className="w-3 h-3" />
                              </Button>
                            </div>
                            {host.phone && (
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="relative flex gap-3 sm:gap-4 items-center">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] sm:text-xs text-gray-500">{format(new Date(booking.created_date), 'MMM d, yyyy HH:mm')}</p>
                    </div>
                  </div>
                  ...
                  {!isAdventureBooking && offers.length > 0 && (
                    <div className="relative flex gap-3 sm:gap-4 items-center">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-xs sm:text-sm">{offers.length} Offer(s) Received</p>
                        <p className="text-[10px] sm:text-xs text-gray-500">{format(new Date(offers[0].created_date), 'MMM d, yyyy HH:mm')}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {acceptedOffer && (
                    <div className="relative flex gap-3 sm:gap-4 items-center">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[10px] sm:text-xs text-gray-500">{format(new Date(acceptedOffer.updated_date || acceptedOffer.created_date), 'MMM d, yyyy HH:mm')}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {booking.status === 'cancelled' && (
                    <div className="relative flex gap-3 sm:gap-4 items-center">
                      ...
                      <div className="flex-1">
                        ...
                        {booking.cancelled_at && (
                          <p className="text-[10px] sm:text-xs text-gray-500">{format(new Date(booking.cancelled_at), 'MMM d, yyyy HH:mm')}</p>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  {booking.status !== 'cancelled' && (
                    <div className="relative flex gap-3 sm:gap-4 items-center">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-xs sm:text-sm">{isAdventureBooking ? 'Adventure Date' : 'Check-in'}</p>
                        <p className="text-[10px] sm:text-xs text-gray-500">{format(new Date(booking.start_date), 'MMM d, yyyy')}</p>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div className="p-3 sm:p-4 border-t border-gray-100 bg-gray-50 flex justify-between items-center">
          <Button variant="outline" size="sm" onClick={() => copyToClipboard(booking.id, 'Booking ID')} className="h-8 text-xs">
            <Copy className="w-3 h-3 mr-1" />
            Copy ID
          </Button>
          <Button onClick={() => onOpenChange(false)} size="sm" className="h-8 text-xs">Close</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
