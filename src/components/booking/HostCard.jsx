...
export default function HostCard({ host }) {
  ...
  const hostInitial = (host.display_name || host.full_name || host.email || '?')
    .charAt(0)
    .toUpperCase();
      ...
      <CardContent className='p-0'>
        <div className='relative h-40'>
          <img
            src={
              host.cover_photos?.[0] ||
              host.profile_photo ||
              `https://source.unsplash.com/400x300/?travel,${host.city || 'destination'}`
            }
        ...
        <div className='p-4 pt-10 relative'>
          ...
          <p className='text-gray-600 text-center text-sm h-10 overflow-hidden'>
            {host.bio?.substring(0, 70) ||
              `Your friendly local guide in ${host.city || 'the city'}.`}
