...
import { Loader2, Sparkles, Wand2, Calendar, Users, Languages, DollarSign, ArrowRight, ArrowLeft, Download, MessageSquare, Trash2, ShieldCheck, Star } from 'lucide-react';
  ...
  <div className="flex justify-center items-center gap-4 sm:gap-8 mb-6">
    {['Details', 'Preview', 'Your Plan'].map((step, index) => (
      <div key={step} className="flex flex-col items-center gap-2">
        <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center border-2 transition-all ${currentStep >= index ? 'bg-purple-600 border-purple-600 text-white' : 'bg-gray-100 border-gray-300 text-gray-400'}`}>
          {currentStep > index ? <ShieldCheck className="w-5 h-5" /> : (index + 1)}
        </div>
        <span className={`text-xs sm:text-sm font-medium transition-colors ${currentStep >= index ? 'text-purple-600' : 'text-gray-500'}`}>{step}</span>
      </div>
    ))}
  </div>
);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <Input id="days" type="number" value={days} onChange={e => setDays(Number(e.target.value))} min="1" max="14" className="h-11" required />
        </div>
        <div>
      ...
      <div>
        ...
        <Textarea id="customPrompt" value={customPrompt} onChange={e => setCustomPrompt(e.target.value)} placeholder="e.g., focus on authentic food, avoid tourist traps, include photo spots..." />
      </div>
      <DialogFooter>
        <Button type="submit" className="w-full sm:w-auto bg-purple-600 hover:bg-purple-700 text-white h-11">
    ...
    {isLoading ? (
      ...
      <>
        ...
        <DialogFooter className="mt-6">
          ...
          <Button onClick={onRegenerate} variant="ghost">Regenerate</Button>
      ...
      <div className="space-y-6">
        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {hosts.map(host => (
                    <div key={host.user_email} className="border rounded-lg p-4 text-center hover:shadow-md transition-shadow">
                        <img src={host.profile_photo || `https://ui-avatars.com/api/?name=${host.display_name}`} alt={host.display_name} className="w-20 h-20 rounded-full mx-auto mb-3 object-cover" loading="lazy" />
                        ...
                        <p className="text-xs text-gray-500 mb-3 line-clamp-2">{host.bio || 'Experienced local host.'}</p>
                        <Button size="sm" className="w-full" onClick={() => handleMessageHost(host.user_email)}>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => window.print()}><Download className="mr-2 w-4 h-4" /> Download Plan</Button>
        </DialogFooter>
      </div>
    </div>
  );
...
export default function AITripPlannerModal({ isOpen, onClose, city }) {
  ...
  const { data: hosts, isLoading: areHostsLoading } = useQuery({
    ...
    queryFn: () => base44.entities.HostProfile.filter(
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {step === 0 && <PlanForm city={city} request={request} setRequest={setRequest} onNext={handleNext} />}
            {step === 1 && <PlanPreview planText={plan?.itinerary_text} onNext={handleNext} onBack={handleBack} isLoading={generatePlanMutation.isPending || isCacheLoading} onRegenerate={handleRegenerate} />}
            {step === 2 && plan && <PlanResult plan={plan} onBack={handleBack} hosts={hosts || []} />}
          </motion.div>
        </AnimatePresence>
      </DialogContent>
    </Dialog>
